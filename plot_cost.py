...
def prepare_agent(in_state=None):
    ...
    net = lasagne.layers.DenseLayer(net,num_units=25,nonlinearity=lasagne.nonlinearities.tanh)
...
def run_bbox(verbose=False):
    ...
    reality = T.vector('scores_n_state')
    ...
    for epoch in range(epochs):
        ...
        while has_next:
            ...
            now = train_fn(memory,actuals) #train based on the score change
            error_cur+= now
            error += now
            ...
            if steps%100==0:
                plot_err = plot_err + [error_cur/100]
                plot_steps = plot_steps + [steps/100]
                plot_err_av = plot_err_av+[error/steps]
                error_cur=0
                if steps%10000==0:
                    plt.plot(plot_steps,plot_err,'k',plot_steps,plot_err_av,'r')
                    plt.grid(True)
                    plt.xlabel('Steps/100')
                    plt.ylabel('Error [red=average, black=local]')
        ...
        score = bbox.get_score()
        name = "Plots/"+time.strftime("%a_%T",time.localtime())+"_Epoch%i.png" % epoch
        plt.axis([0, max(plot_steps), 0, max(plot_err)])

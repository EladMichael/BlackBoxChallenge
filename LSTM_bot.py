...
def prepare_agent(in_state=None):
    net = lasagne.layers.InputLayer(shape=(memtime,1,n_f+2),input_var=in_state)
    net = lasagne.layers.dropout(net,p=.1)
    net = lasagne.layers.LSTMLayer(net,num_units=20)
    net = lasagne.layers.dropout(net,p=.5)
    net = lasagne.layers.DenseLayer(net,num_units=20,nonlinearity=lasagne.nonlinearities.tanh)
    net = lasagne.layers.dropout(net,p=.5)
...
def forget(states):
    for row in range(len(states)-1,0,-1):
        states[row][0]=states[row-1][0]
    states[0][0] = np.zeros(shape=(1,n_f+2))
    return states

...
def get_best_action(state=None):
    scores = get_all_score_diffs(state)
    action_to_do = np.argmax(scores)
    actions = np.zeros(n_a)
    actions[action_to_do]=1
...
def run_bbox(verbose=False):
    ...
    for epoch in range(epochs):
        ...
        while has_next:
            ...
            train_fn(memory,best) #train based on the score change
            ...
            if steps%10000==0:
                ...
                print ("   local assessment: {}%\ correct".format(float(good)/100))
        ...
        with open("epoch_data.txt","a") as f:
        	f.write("Epoch: {}    Final Score: {}    Time to Run: {} min\n".format(epoch,score,(time.time()-e_time)/60))
